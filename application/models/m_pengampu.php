...
class M_Pengampu extends CI_Model
{
	...
	function get($semester_tipe, $tahun_akademik)
	{
		...
		$sql  = "SELECT a.kode as kode," .
			...
			"       d.kode as `kode_asdos`," .
			"       d.nama_asdos as  `nama_asdos`," .
			"       a.kelas as kelas," .
	...
	function joinData()
	{
		...
		$this->db->join('matakuliah', 'pengampu.kode_mk=matakuliah.kode');
		$this->db->join('dosen', 'pengampu.kode_dosen=dosen.kode');
		$this->db->join('asdos', 'pengampu.kode_asdos=asdos.kode');
		$this->db->order_by('pengampu.kode', 'ASC');
		$query = $this->db->get();
		return $query->result();
	}
	...
	function cek_for_update($kode, $kode_mk, $kode_dosen, $kelas, $tahun_akademik, $id)
	{
		$rs = $this->db->query(
