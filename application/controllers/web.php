...
class Web extends CI_Controller
{
	...
	function index()
	{
		if ($this->session->userdata('Login')) {
			...
			$data['page_name'] = 'VBlank';
			$data['jumlahDataDosen'] = $this->m_dosen->jumlahDosen();
			$data['jumlahDataMatkul'] = $this->m_matakuliah->jumlahMatkul();
			$data['jumlahDataRuangan'] = $this->m_ruang->jumlahRuangan();
			$this->load->view('VBackend', $data);
		} else {
		...
		if (!empty($_POST)) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if (IS_TEST === 'FALSE') {
					...
					$data['msg'] = ' <strong>Success</strong> Data Berhasil Ditambahkan';
					$this->session->set_flashdata(
						'msg',
						'<strong>Sukses</strong> Data berhasil Tambahkan.
					'
					);
					$data['clear_text_box'] = 'TRUE';
					redirect(site_url('web/dosen', $data));
				} else {
	...
	function dosen_edit($kode)
	{
		...
		$data['page_title'] = 'Update Data Dosen';
	...
	function matakuliah_add()
	{
		...
		if (!empty($_POST)) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if (IS_TEST === 'FALSE') {
					...
					$data['clear_text_box'] = 'TRUE';
					redirect(site_url('web/matakuliah'));
				} else {
					$data['msg'] = 'WARNING: READ ONLY !';
				}
			} else {
		...
		$data['page_title'] = 'Tambah Matakuliah';
	...
	function matakuliah_edit($kode)
	{
		...
		$data['page_title'] = 'Update Data Matakuliah';
	...
	function ruang_add()
	{
		...
		$data['page_title'] = 'Form Tambah Ruang';
	...
	function ruang_edit($kode)
	{
		...
		$data['page_title'] = 'Form Edit Ruang';
	...
	function jam_add()
	{
		...
		$data['page_title'] = 'Form Tambah Range Jam';
	...
	function jam_edit($kode)
	{
		...
		$data['page_title'] = 'Form Edit Range Jam';
	...
	{
		$data = array();
		$data['page_title'] = 'Data Asiten Dosen';
		$data['rs_asdos'] = $this->m_asdos->get();
		$data['page_name'] = 'asdos';
		$this->render_view($data);
	}
	...
	function asdos_add()
	{
		...
		if (!empty($_POST)) {
			...
			$this->form_validation->set_rules('nama_asdos', 'Nama Asdos', 'xss_clean|required|is_unique[asdos.nama_asdos]');

			if ($this->form_validation->run() == TRUE) {
				$data['nama_asdos'] = $this->input->post('nama_asdos');


				if (IS_TEST === 'FALSE') {
					$this->m_asdos->insert($data);
					$data['msg'] = 'Data Telah Berhasil Ditambahkan';
					$this->session->set_flashdata(
						'msg',
						'<strong>Sukses</strong> Data berhasil Tambahkan.
		...
		$data['page_name'] = 'asdos_add';
		$data['page_title'] = 'Form Tambah Asisten Dosen';

		$this->render_view($data);
	}
	function asdos_edit($kode)
	{
		$data = array();

		if (!empty($_POST)) {

			$this->form_validation->set_rules('nama_asdos', 'Nama_asdos Hari', 'xss_clean|required');

			if ($this->form_validation->run() == TRUE) {
				$data['nama_asdos'] = $this->input->post('nama_asdos');

				if (IS_TEST === 'FALSE') {
					$this->m_asdos->update($kode, $data);
					$data['msg'] = 'Data telah berhasil dirubah';
					$this->session->set_flashdata(
						'msg',
						'<strong>Sukses</strong> Data berhasil Diubah.
		...
		$data['page_name'] = 'asdos_edit';
		$data['page_title'] = 'Form Edit Hari';
		$data['rs_asdos'] = $this->m_asdos->get_by_kode($kode);

		$this->render_view($data);
	}
	function asdos_delete($kode)
	{

		if (IS_TEST === 'FALSE') {
			$this->m_asdos->delete($kode);
			$this->session->set_flashdata('msg', 'Data telah berhasil dihapus');
			$this->session->set_flashdata(
				'msg',
				'<strong>Sukses</strong> Data berhasil dihapus.
		...
		redirect(base_url() . 'web/asdos', 'reload');
	}
	/**************************************************************************************************/
	...
	function hari_add()
	{
		...
		$data['page_title'] = 'Form Tambah Hari';
	...
	function pengampu($semester_tipe = null, $tahun_akademik = null)
	{
		...
		*/
		...
		if (!$this->session->userdata('pengampu_semester_tipe') && !$this->session->userdata('pengampu_tahun_akademik')) {
			...
			$this->session->set_userdata('pengampu_tahun_akademik', '2022-2023');
		}
	...
	function pengampu_add()
	{
		...
		if (!empty($_POST)) {
			...
			$this->form_validation->set_rules('kode_asdos', 'Asdos', 'xss_clean|required');
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$data['kode_asdos'] = $this->input->post('kode_asdos');
				...
				if (IS_TEST === 'FALSE') {
					...
					'
						...
						redirect(site_url('web/pengampu'));
					} else {
	...
	function pengampu_edit($kode)
	{
		...
		$data['page_title'] = 'Form Update Pengampu';
		$data['rs_pengampu'] = $this->m_pengampu->get_by_kode($kode);

		$data['rs_mk'] = $this->m_matakuliah->get_all();
		$data['rs_dosen'] = $this->m_dosen->get_all();
		$data['rs_asdos'] = $this->m_asdos->get_all();
	...
	function penjadwalan()
	{
		...
		$data = [];
		// $genetik = new Genetik(null, null, null, null, null, null, null, null);
		global $genetik;
		if (!empty($_POST)) {
			$this->form_validation->set_rules('semester_tipe', 'Semester', 'xss_clean|required');
			$this->form_validation->set_rules('tahun_akademik', 'Tahun Akademik', 'xss_clean|required');
		...
		// $produk->tambahProduk($genetik);
		// $produk = new CetakInfoProduk($genetik);
		// ($produk->getMethod($genetik));
		// $metod = $produk->getMethod(ReflectionMethod::IS_PUBLIC);
		// var_dump($metod);
		// $produk->tambahProduk();
		// $data['produk'] = $produk;
		// $data['produk1'] = $cetak;
		...
		// $data['aku'] = $produk->tambahProduk($metod);

		// $tampil = $this->m_jadwalkuliah->get()->row();
		// // proses
		// $data['pengampu'] = $genetik->pengampu();
		// $data['jumlah_jam'] = $genetik->jumlah_jam();
		// $data['jumlah_hari'] = $genetik->jumlah_hari();
		// $data['ruangan'] = $genetik->ruangan();

		// $data['fitness'] = $fitness;

		// $data['jumlahRank'] = $genetik->jumlahRank();
		// $data['induk'] = $genetik->induk();
		// $data['rank'] = $genetik->rank();
		// $data['fitness'] = $genetik->fitness();

		// $data['pengampu'] = $genetik->pengampu();
		// $data['populasi'] = $genetik->populasi();
		// $data['individu'] = $genetik->individu();
		// $data['skss'] = $genetik->sks();
		// $data['jum_jam'] = $genetik->jumlah_jam();
		// $data['jum_hari'] = $genetik->jumlah_hari();
		// $data['ruangan'] = $genetik->ruangan();
		// $data['ruanganR'] = $genetik->ruanganR();
		// $data['individu_baru'] = $genetik->individuBaru();
		// $data['mutasi'] = $fitnessAfterMutation;
		// $data['crossover'] = $genetik->StartCrossOver();

		// $data['dosen'] = $tampil->dosen;
		// $data['hitungfitness'] = $genetik->HitungFitness();
		// $data['seleksi'] = $genetik->Seleksi($fitness);
		// $data['GetIndividu'] = $jadwal_kuliah;
		// $data['AmbilData'] = $genetik->AmbilData();
		// $data['aku'] = $genetik->dosen();
		// $data['populasi'] = $genetik->populasi();
		// $data['individuSolusi'] = $genetik->individuSolusi();
		// $data['Inisialisai'] = $genetik->Inisialisai();
		// // $data['StartCrossOver'] = $genetik->StartCrossOver();

		// $data['individu'] = $genetik->individu();
		// $data['individu'] = $genetik->Inisialisai()->jumlah_pengampu;
		$data['namaDosen'] = $this->m_pengampu->joinData();
